...
void Tetromino::Rotate() {
  ...
  old_coords.first =
      old_coords.first -
      TetroCoords::coordsTable[static_cast<int>(shape_)]
                              [static_cast<int>(position_)][0][1];
  old_coords.second =
      old_coords.second -
      TetroCoords::coordsTable[static_cast<int>(shape_)]
                              [static_cast<int>(position_)][0][0];
  // for (int i = 0; i < 4; i++) {
  //   int x = t_coords_[i].x - (center_x);
  //   int y = t_coords_[i].y - (center_y);
  //   int newX = rotationMatrix[0][0] * x + rotationMatrix[0][1] * y +
  //   (center_x); int newY = rotationMatrix[1][0] * x + rotationMatrix[1][1] *
  //   y + (center_y); t_coords_[i] = {newX, newY};
  // }

  SetShape(shape_,
           static_cast<TetroPosition>(((static_cast<int>(position_)) + 1) % 4));
  // int difX = t_coords_[0].x + old_coords.first;
  // int difY = t_coords_[0].y + old_coords.second;
  for (int i = 0; i < 4; i++)
    t_coords_[i] = {t_coords_[i].x + old_coords.first,
                    t_coords_[i].y + old_coords.second};
  // t_coords_ = newCoords;
  // while (GetMinY() <= 0) {
  //   MoveDown();
  // }
...
void Tetromino::SetShape(TetroShape s, TetroPosition p) {
  ...
  position_ = p;
  for (int i = 0; i < 4; i++) {
    t_coords_[i].x = TetroCoords::coordsTable[static_cast<int>(s)]
                                             [static_cast<int>(p)][i][1];
    t_coords_[i].y = TetroCoords::coordsTable[static_cast<int>(s)]
                                             [static_cast<int>(p)][i][0];
  }
}
